...
type AttendanceFormProps = {
  ...
  /*
   * Initial date supplied by the Server Component.
   * Usually today's date.
   */
  initialDate: Date;

  /*
   * Attendance for initialDate.
   */
  attendanceTaken: boolean;
  todayAttendance: AttendanceMap;

  /*
   * Server Action callback used when
   * the client changes the date.
   */
...
export function AttendanceForm({
  ...
  const handleDateChange = async (
    ...
    /*
     * Close any dialogs from the previous date.
     */
    setConfirmationOpen(false);
    setAttendanceAlreadySubmitted(false);

    /*
     * Clear old attendance immediately so
     * the previous date's data isn't displayed
     * while the new date is loading.
     */
    setAttendance({});
    ...
    try {
      ...
      /*
       * Keep the UI safe if loading fails.
       */
      setAttendance({});
      setAttendanceTaken(false);
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      /*
       * IMPORTANT:
       * Selected date is explicitly sent to
       * the Server Action.
       */
  ...
  const presentCount = students.filter(
    (student) =>
      getStatus(student.userId) === "PRESENT"
  ).length;

  const leaveCount = students.filter(
    (student) =>
      getStatus(student.userId) === "LEAVE"
  ).length;

  const absentCount = students.filter(
    (student) =>
      getStatus(student.userId) === "ABSENT"
  ).length;
