...
/*
 * tocItems are needed by PatternTocRangeFields on the client,
 * but they are not needed by saveGlobalLearnings().
 */
const toGlobalLearningPayload = (
  ...
  pattern: learning.pattern
    ? {
        ...
        patternArr: learning.pattern.patternArr,
      }
    : null,
...
const [studentProgressSearch, setStudentProgressSearch] =
  useState("");
...
const [progressLoading, setProgressLoading] = useState(true);
 ...
 const buildProgressLearnings = (learnings: Learning[]) =>
...
const submitStudent = async (student: Student) => {
  ...
  if (!data || data.learnings.length === 0) {
    return;
  }

  const learnings = buildProgressLearnings(data.learnings);
  ...
  try {
    ...
    setProgress((current) => ({
      ...
      [student.id]: {
        ...current[student.id],
        learnings: current[student.id].learnings.map(
          (learning) => ({
            ...learning,
            saved: true,
          })
    ...
    try {
      ...
      setProgress((current) => {
        ...
        for (const saved of savedProgress) {
          ...
          const savedLearnings = Array.isArray(saved.learnings)
            ? saved.learnings
            : [];

          const learnings: Learning[] = savedLearnings.map(
            (learning: any) => ({
 ...
 <Collapsible.Root defaultOpen={false}>
  ...
  onClick={async () => {
    try {
      ...
      const learnings = globalLearnings.map(
      ...
      >
        ...
        <div className="relative w-full md:w-72 lg:w-80">
          ...
          <input
            ...
            onChange={(event) =>
              setStudentProgressSearch(event.target.value)
            }
          ...
          <p className="text-xs text-muted-foreground">
            {students.filter((student) =>
              student.name
                .toLowerCase()
                .includes(
                  studentProgressSearch.trim().toLowerCase()
                )
            ).length}{" "}
            student
            {students.filter((student) =>
